...
public class JumpResultSynchronizeSchedulerTest {
    ...
    public void synchronizeDataWhenExistTest() throws InternalServiceException, IOException {
        //setup
        Long raceDataId = 1L;
        DataRaceDTO dataRaceDTO = dataRaceService.save(new DataRaceDTO().date(LocalDate.now()).city("krk").shortCountryName("name").raceId(raceDataId));
        SkiJumperDTO firstSkiJumperDTO = skiJumperService.save(new SkiJumperDTO().bib(28).fisCode(4646).name("Jurij Tepes"));
        SkiJumperDTO secondSkiJumperDTO = skiJumperService.save(new SkiJumperDTO().bib(30).fisCode(5658).name("Peter Prevc"));
        SkiJumperDTO thirdSkiJumperDTO = skiJumperService.save(new SkiJumperDTO().bib(25).fisCode(5987).name("Rune Velta"));

        JumpResultDTO firstExpectedJumpResultDTO = new JumpResultDTO()
                .id(1L)
                .rank(1)
                .jumperId(firstSkiJumperDTO.getId())
                .dataRaceId(dataRaceDTO.getRaceId())
                .firstJump(230.5)
                .pointsForFirstJump(208.9)
                .secondJump(244.0)
                .pointsForSecondJump(227.5)
